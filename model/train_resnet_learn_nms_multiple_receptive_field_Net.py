...
import argparse
import nets.RC3D_resnet_learn_nms_multi_receptive_field as RC3D_resnet
...
def arg_parse():
    parser = argparse.ArgumentParser(description = "ResnetNetNMS_MUL")
    parser.add_argument("--extract_feature", dest = 'extract_feature', type = str, default = 'True')
...
def train(args):
    ...
    if args.use_resnet_pth == 'True':
        ...
        step = 0
        ckpt_path = os.path.join(args.checkpoint_path, "MULResNetNMS.ckpt")
    else:
        try:
            names = os.listdir(ckpt_path)
            for name in names:
                out = re.findall("MULResNetNMS_.*.ckpt", name)
                if out != []:
                    ckpt_path = out[0]
                    break
    ...
    while step < args.iters:
        ...
        if step % args.snapshot_per_iters == 0 and step:
            try:
                ...
                pass
            ckpt_path = os.path.join(args.checkpoint_path, "MULResNetNMS_{:05d}.ckpt".format(step))
